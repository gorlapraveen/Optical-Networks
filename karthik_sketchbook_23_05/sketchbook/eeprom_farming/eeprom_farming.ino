...
String authorizedCaller(String caller) {
  ...
  //  EEPROM.read( f*(PhoneNumberSize+PhoneNameSize) );
  ...
  if (found != -1) {
    //EEPROM.read( (found*(PhoneNumberSize+PhoneNameSize))+PhoneNumberSize );
    return phoneName;
  } else {
    return "";
  }
...
String phoneBook() {
  ...
  do {
   // EEPROM.read( f*(PhoneNumberSize+PhoneNameSize+1) );
  //  EEPROM.read( (f*(PhoneNumberSize+PhoneNameSize+1))+PhoneNumberSize );
   // EEPROM.read( (f*(PhoneNumberSize+PhoneNameSize+1))+PhoneNumberSize+PhoneNameSize );
   // gotNumber = "";
...
String addPhone(String command, bool admin) {
  String strNumber="8008645273";
  String strName="karthik";
  String result;
  char phoneNumber[PhoneNumberSize];
  char phoneName[PhoneNameSize];
  char phoneAdmin;
 ...
 //   EEPROM.write( f*(PhoneNumberSize+PhoneNameSize+1) );
   // EEPROM.write( (f*(PhoneNumberSize+PhoneNameSize+1))+PhoneNumberSize );
   // EEPROM.write( (f*(PhoneNumberSize+PhoneNameSize+1))+PhoneNumberSize+PhoneNameSize );
    result = "New entry: ";
...
String delPhone(String command) {
  ...
  if (strNumber != "") {
    ...
    if (found != -1) {
      phoneNumber[0]=0;
     // EEPROM.write( found*(PhoneNumberSize+PhoneNameSize+1) );
